"""EDEKA Offers image platform for PAYBACK / Loyalty Card QR code."""
...
async def async_setup_entry(
    ...
    """Set up EDEKA Loyalty Card QR Code image entity from a config entry."""
    ...
    if coordinator.card_number and coordinator.account_key not in created_images:
        ...
        async_add_entities(
            [EdekaLoyaltyCardQrImage(hass, coordinator)], update_before_add=False
        )


class EdekaLoyaltyCardQrImage(
    CoordinatorEntity[EdekaDataUpdateCoordinator], ImageEntity
):
    """Represents the EDEKA PAYBACK / Loyalty card QR code image entity."""
    ...
    def __init__(
        self, hass: HomeAssistant, coordinator: EdekaDataUpdateCoordinator
    ) -> None:
        ...
        self._attr_unique_id = f"edeka_{self._account_key}_loyalty_card_qr"
